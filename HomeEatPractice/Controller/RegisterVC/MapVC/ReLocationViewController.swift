...
class ReLocationViewController : UIViewController, UITextFieldDelegate, UIScrollViewDelegate {
            ...
            do {
                ...
                for neighborhood in decodedData.data.neighborhoods{
                    ...
                    let buttonAction = UIAction{ _ in
                        ...
                        UserDefaults.standard.setValue(neighborhood.fullNm, forKey: "userAddress")
                        print(UserDefaults.standard.value(forKey: "selectedLocation"))
                        self.navigationController?.pushViewController(MapViewController2(), animated: true)
                    }
                    let customButton = UIButton(configuration: config, primaryAction: buttonAction)
